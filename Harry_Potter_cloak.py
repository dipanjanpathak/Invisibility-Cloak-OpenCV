...
while(True):
	ret,frame = cap.read()  # gives two output 1. ret ==> true or false for if a frame was successfully read or not. 2.frame (numpy.ndarray ==> A matrix (array) representing the image frame in BGR format. 
	inspect = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)  # used to convert an image from one color space to another. In this specific case, it converts an image from the BGR color space (default in OpenCV) to the HSV color space.

	#getting the HSV values for masking the cloak
	upper_hue = cv2.getTrackbarPos("upper_hue", "bars")   # it retrieves the current position (value) of a trackbar (slider) named "upper_hue" within a window named "bars".
	...
	kernel = numpy.ones((3,3),numpy.uint8)  # it creates a 3 X 3 matrix of 1. it will use to reduce noise
